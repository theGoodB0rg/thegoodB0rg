...
TECH_START = "<!--START_SECTION:tech_stack-->"
TECH_END = "<!--END_SECTION:tech_stack-->"
...
def load_tech_stack(path="tech_stack.json"):
    if not os.path.exists(path):
        return []
    try:
        with open(path, "r", encoding="utf-8") as f:
    ...
    sanitized = []
    for item in data:
        alt = (item.get("alt") or item.get("title") or "").strip()
        src = (item.get("src") or item.get("badge") or "").strip()
        if not alt or not src:
            continue
        link = (item.get("link") or item.get("href") or "").strip()
        attrs = {}
        for key in ("height", "width", "loading", "style", "title", "class"):
            value = item.get(key)
            if value is None:
                continue
            text_value = str(value).strip()
            if text_value:
                attrs[key] = text_value
        sanitized.append({
            "alt": alt,
            "src": src,
            "link": link,
...
def build_tech_section(items):
    ...
    for item in items:
        alt = escape(item["alt"])
        src = escape(item["src"])
        attrs = [f'alt="{alt}"', f'src="{src}"']
        for key, value in item.get("attrs", {}).items():
            attrs.append(f'{key}="{escape(value)}"')
        badge_html = f"<img {' '.join(attrs)}>"
        link = item.get("link")
        if link:
            link_href = escape(link)
            badge_html = f'<a href="{link_href}" target="_blank" rel="noopener noreferrer">{badge_html}</a>'
...
def main():
    ...
    tech_items = load_tech_stack()
    if tech_items:
        tech_block = build_tech_section(tech_items)
        tech_updated = replace_between_markers(updated, tech_block, TECH_START, TECH_END)
        if tech_updated is None:
            print("Tech stack markers not found in README.md; skipped updating tooling badges.")
        else:
            if tech_updated != updated:
                changed = True
            updated = tech_updated
    else:
        print("No tech stack data found; skipped updating tooling badges.")
